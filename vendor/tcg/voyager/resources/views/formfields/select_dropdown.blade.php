...
@else
    ...
    <select class="form-control select2" name="{{ $row->field }}">
        ...
        @if(isset($options->options))
            @foreach($options->options as $key => $option)
                <option value="{{ $key }}" @if($default == $key && $selected_value === NULL) selected="selected" @endif @if($selected_value == $key) selected="selected" @endif>{{($key == '0' && $dataType->slug =='comments')  ? __('generic.wait_confirm') : __('generic.confirmed') }}</option>
            @endforeach
        @endif
    </select>
@endif
