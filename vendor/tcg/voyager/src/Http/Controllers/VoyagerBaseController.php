...
class VoyagerBaseController extends Controller
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        if($slug == 'products')
        {
            ...
            $data->load('translations');$catagory->load('translations');$filters->load('translations');$filtervalues->load('translations');
    ...
    public function update(Request $request, $id)
    {
        ...
        if($slug == 'products')
        {
            $num = (int)str_replace(',', '', $request->price);
            $request->request->remove('price');
            $request->request->add(['price'=>$num]);
        }
        ...
        if($slug == 'products')
        {
           /*
            ...
            $request->request->remove('catagory1');
            $request->request->add(['catagory1'=>$cat1->name]);
            $cat2 = $request->catagory2;
            $cat2 = Category::find($cat2);
            if($cat2){
                //$newproduct->catagory2 = $cat2->name ? $cat2->name : '';
                $request->request->remove('catagory2');
                $request->request->add(['catagory2'=>$cat2->name]);}

            $cat3 = $request->catagory3;
            $cat3 = Category::find($cat3);
            if($cat3){
                // $newproduct->catagory3 = $cat3->name ?? '';
                $request->request->remove('catagory3');
                $request->request->add(['catagory3'=>$cat3->name]);}
           ...
           $result = $this->insertUpdateData($request, $slug, $dataType->editRows, $data);
            ...
            if($result == 'error')
            {
               ...
               return redirect()->back()->with([
                   'message'    => __('generic.can_not_empty_filter'),
               ]);

            }
            else if($result == 'no_image')
                return redirect()->back()->with([
                    'message'    => __('generic.require_one_image'),
                ]);
            else
            {
        ...
        elseif ($slug == 'filters')
        {
            $data2 = $this->insertUpdateData($request, $slug, $dataType->editRows, $data);
           // $objfilter = Filter::findOrFail($id);
            ...
            {
                $filtervaluesjson = $request->value_i18n;
                $filtervaluedecoded = json_decode($filtervaluesjson);
                $filtervalue_fa = $filtervaluedecoded->fa;
                $filtervalue_en = $filtervaluedecoded->en;
                $filtervalue_fa = explode("\n",$filtervalue_fa);
                $filtervalue_en = explode("\n",$filtervalue_en);
                //foreign_key
                $findfiltervalueids = Filtervalue::where('filter_id',$id)->get();
                foreach ($findfiltervalueids as $deleteTrans)
                    DB::table('translations')->where('foreign_key' ,'=' ,$deleteTrans->id)->delete();
                Filtervalue::where('filter_id',$id)->delete();
                for ($i = 0 ;$i < count($filtervalue_fa) -1 ;$i ++)
                {
                    $request = new Request();
                    $request->setMethod('POST');
                    if(isset($filtervalue_en[$i]))
                    {
                        $newfiltervalue = new Filtervalue();
                        $extractfiltervalue = array('fa' => $filtervalue_fa[$i] ,'en' => $filtervalue_en[$i]);
                        $valecodeded = json_encode($extractfiltervalue);
                        $request->request->add(['value_i18n' => $valecodeded]);
                        $request->request->add(['filter_id' => $data2->id]);
                        // exit("filter values is " .$request->input('value_i18n'));
                        $dataType_filtervalue = Voyager::model('DataType')->where('slug', '=', 'filtervalues')->first();

                        $translations = is_bread_translatable($newfiltervalue) ? $newfiltervalue->prepareTranslations($request)
                            : [];
                        $datafiltervalue = $request->request->remove('value_i18n');
                        // $newfiltervalue = new Filtervalue();
                        $newfiltervalue->value = $filtervalue_fa[$i];
                        $newfiltervalue->filter_id = $data2->id;
                        $newfiltervalue->save();
                        if (count($translations) > 0) {
                            $newfiltervalue->saveTranslations($translations);
                        }

                    }
                    else
                        return redirect()->back()->with([
                            'message'    => __('voyager::generic.error_updating')." {$dataType->getTranslatedAttribute('display_name_singular')}",
                            'alert-type' => 'info',
                        ]);
...
//---------------------
    ...
    public function create(Request $request)
    {
        ...
        //---------------------------------------------------------------------------------------------
        if($slug == 'products')
            return redirect('admin/products/createProduct')->with(['dataType' => $dataType ,'dataTypeContent' => $dataTypeContent ,'isModelTranslatable' => $isModelTranslatable]);
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
    }
    ...
    public function store(Request $request)
    {
      ...
      //  $this->authorize('add', app($dataType->model_name));
        ...
        if($slug == 'products')
        {
            //$request->request->add($request->old());
           ...
           // $res = $newproduct->save();
            ...
            if($request->filter ){
                ...
                foreach ($request->filter as $r)
                {
                    ...
                    $data->productfilters()->save($objfilter);
            ...
            if(isset($request->image) )
            {
                ...
                {
                            $newimageproduct = new Productimage();
                            $newimageproduct->image = $img;
                            $f = $data->productimages()->save($newimageproduct);
                }
            }
...
//---------------------
        ...
        elseif ($slug == 'filters')
        {
           ...
           // $data = $this->insertUpdateData($request, $slug, $dataType->addRows, new $dataType->model_name());
            ...
            if(isset($request->filtervalues))
            {
                ...
                $i = 0 ;
                foreach ($filtervalue_fa as $filtervalue)
                {
                    $request = new Request();
                    $request->setMethod('POST');
                    if(isset($filtervalue_en[$i]) && $filtervalue_en[$i])
                    {
                        $newfiltervalue = new Filtervalue();
                        $extractfiltervalue = array('fa' => $filtervalue ,'en' => $filtervalue_en[$i]);
                        $i++ ;
                        $valecodeded = json_encode($extractfiltervalue);
                        $request->request->add(['value_i18n' => $valecodeded]);
                        $request->request->add(['filter_id' => $data->id]);
                        // exit("filter values is " .$request->input('value_i18n'));
                        $dataType2 = Voyager::model('DataType')->where('slug', '=', 'filtervalues')->first();
                        $translations = is_bread_translatable($newfiltervalue) ? $newfiltervalue->prepareTranslations($request)
                            : [];
                        $datafiltervalue =
                        $request->request->remove('value_i18n');
                        // $newfiltervalue = new Filtervalue();
                        $newfiltervalue->value = $filtervalue;
                        $newfiltervalue->filter_id = $data->id;
                         $newfiltervalue->save();
                        if (count($translations) > 0) {
                            $newfiltervalue->saveTranslations($translations);
                        }
                        ...
                        return redirect()->back()->with([
                    'message'    => __('voyager::generic.error_storing')." {$dataType->getTranslatedAttribute('display_name_singular')}",
                    'alert-type' => 'info',
                ]);
    ...
    {
        // save filtervalue id for delete translation table fild
        $findfiltervalueids = Filtervalue::where('filter_id',$id)->get();
        $slug = $this->getSlug($request);
        ...
        if ($res) {
            //delete data form translation table for filter id
