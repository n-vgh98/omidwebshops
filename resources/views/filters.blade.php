    ...
    @foreach($filters as $filter)
        <div class="container-fluid"  style="font-size: large;border-style: solid;border-color:#0c5460;border-width: 1px ">
                ...
                <div class=" row" >
                    <div class="col-sm-12">
                       <div class="checkbox">
                        <label ><input type="checkbox" name="filter[]" value='<?php echo  "$filter->id"; ?>' ><strong class="text-danger">{{$filter->getTranslatedAttribute('name')}} : </strong></label>
                    </div>
                    @php
                        $id = $filter->id;
                    ...
                    @foreach($filterval as $fv)
                        <label class="radio-inline" style="margin-right:20px">
                            ...
                            {{$fv->getTranslatedAttribute('value')}}
                        </label>
                    @endforeach
                             ...
                             <div class="form-group">
                                 <label class="radio-inline" style="margin-right:20px">
                                     ...
                                     {{__('product.favoriate')}} <input type="text" class="form-control" id="<?php echo 'txtradio'.$filter->id; ?>" onkeyup="addvaluetoradionbutton({{$filter->id}})" >
                                 </label>
                             </div>
