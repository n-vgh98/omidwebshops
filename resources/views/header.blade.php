...
<div class="languag   <?php if (__('generic.is_rtl') == 'true') echo 'ltr'; else echo 'rtl' ?>">
    <a href="{{url('fa')}} " class="p-1" style="background-color: #1cc88a">Fa</a><a class="p-1"
...
<div class="container-fluid">
    <div class="row">
        <div class="col-2">
            <div class="">
                <a href="/" title="لوگو">
                    <img src="{{asset('images/shortcutIcon.png')}}" alt="{{__('generic.logo')}}"
        ...
        <div class="col-6">
            <div class="searchBox  mt-4  mr-auto ">
                <form class="form-inline" action="{{url('search')}}" method="post">
                    @csrf
                    <input class="form-control p-2 ml-2 rounded " type="search" placeholder="{{__('generic.search')}}"
                ...
                <div>
                    @if(\Illuminate\Support\Facades\Auth::check())
                        ...
                        <div class="dropdown  profile" role="presentation">
                            ...
                            <a class="dropdown-toggle no-arrow" data-toggle="dropdown" aria-expanded="true" href="#">
                                <span
                                    class="d-none d-lg-inline  text-secondary small">{{\Illuminate\Support\Facades\Auth::user()->name}}</span>
                            ...
                            <div class="dropdown-menu shadow   dropdown-menu-animated  border-secondary " role="menu"
                                ...
                                <a class="dropdown-item text-right" role="presentation" href="{{url('sabadKharid')}}">
                                    <i class="fas fa-shopping-basket fa-sm fa-fw text-gray-400"></i>&nbsp; {{__('generic.bag_cart')}}
                                </a>
                    ...
                    @else
                        <a class="btn btn-outline-primary text-dark ml-auto " value="ورود" name="btnLogin"
                           href="{{route('login')}}">{{__('generic.login')}}</a>
                    @endif
                </div>
        ...
        <ul class="navbar-nav nav-pills">
            ...
            @foreach($root as $item)
                ...
                <li class="nav-item dropdown position-static">
                    <a class="nav-link" @if($item->name == __('generic.home')) href="/"
                       @else href="/product/catagory/{{$item->name}}" @endif id="menu1dropdown">
                        {{--  {{$item->name --}}
