...
@section('content')
    ...
    <div class="container bg-white mt-3">
        <div class="row justify-content-center">
            <div class="col-4">
                <h3 class="text-center  border text-danger">{{__('generic.bag_buy')}}</h3>
            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-6 my-2">
                <div class="form-group">
                    <a class="btn btn-dark " href="/">{{__('generic.add_product')}}</a>
                    <a class="btn btn-danger " href="/empty/cart">{{__('generic.empty_card')}}</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="row bg-light mt-2">
            <div class="col-sm-4 border-left text-center">
                <span>{{__('generic.name_product')}}</span>
            </div>

            <div class="col-sm-1 border-left text-center">
                <span>{{__('generic.number')}}</span>
            </div>

            <div class="col-sm-2 border-left text-center">
                <span>{{__('generic.price_unit')}}</span>
            </div>

            <div class="col-sm-4 border-left text-center">
                <span>{{__('generic.price_total')}}</span>
            </div>
...
@if(session()->has('cartItem') && count(session('cartItem')) >0 )
        ...
        <div class="row bg-white mt-2">
            ...
            <div class="col-sm-2 border-left d-flex">
                <div class="my-auto mx-auto">
                    ...
                    <span> {{ number_format( $item['price'] ) }}  {{__('generic.vahed_pool')}} </span>
                </div>
            </div>

            <div class="col-sm-4 border-left d-flex">
                <div class="my-auto mx-auto">
                      {{__('generic.price')}}  <span id="sum{{$item['id']}}" class="sum">{{ number_format( $item['price'] * $item['number'] )}} </span>

                    <span>{{__('generic.vahed_pool')}}</span>
                    <div class=" text-danger">
                           {{__('generic.discount')}}  :  <span id="takhfif{{$item['id']}}" class="takhfif"> {{number_format( ( ($item['price'] * $item['takhfif'])/100 ) * $item['number'] )}} </span>

                        <span>{{__('generic.vahed_pool')}}</span>
                    </div>
                    <div class="bg-danger"><hr></div>
                    <div class="">
                         {{__('generic.price_total')}} :  <span class="finalPrice">{{number_format ( ($item['price'] * $item['number'])-( ($item['price'] * $item['takhfif'])/100 ) * $item['number'] )}}</span>{{__('generic.vahed_pool')}}
                    </div>
                    <div>
        ...
        <div class="row justify-content-center">
            <div class="col-6 offset-6 my-2">
                <form method="get" action="/continuePardakht">
                    ...
                    <div class="form-group">
                        <input type="submit" value="{{__('generic.confirm_and_continue_buy')}}" name="btnPardakht" class="btn btn-danger">
                    </div>
                </form>
            </div>
        </div>
