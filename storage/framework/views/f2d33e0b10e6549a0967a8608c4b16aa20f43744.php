                            ...
                            <?php $__currentLoopData = $dataTypeRows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <div class="form-group <?php if($row->type == 'hidden'): ?> hidden <?php endif; ?> col-md-<?php echo e($display_options->width ?? 12); ?> <?php echo e($errors->has($row->field) ? 'has-error' : ''); ?>" <?php if(isset($display_options->id)): ?><?php echo e("id=$display_options->id"); ?><?php endif; ?>>
                                    ...
                                    <?php elseif($row->type == 'relationship'): ?>
                                        <?php
                                        $model = 'App\Filtervalue';
                                        $query = $model::where($row->details->column, '=', $dataTypeContent->{$row->details->key})->get();
                                        echo "<span style='font-style:smal'>(هر مقدار در یک خط قرار دارد.)</span>";
                                        if(isset($query))
                                            {
                                                echo '<textarea class="form-control" rows="7" name=filtervalues>' ;
                                                    foreach($query as $query_res)
                                                        {
                                                           echo $query_res->{$row->details->label};
                                                           echo "\n";
                                                        }
...
<?php echo $__env->make('voyager::master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH F:\xampp\htdocs\shop-laravel\resources\views/vendor/voyager/filters/edit-add.blade.php ENDPATH**/ ?>